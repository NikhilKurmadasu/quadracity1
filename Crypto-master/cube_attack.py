...
class CubeAttack(object):
        ...
        for assign1 in assign_strings:
            for assign2 in assign_strings:
                ...
                if lhs != rhs:
                    ...
                    quadracity_test = True

                    q_flag = False

                    for assign1 in assign_strings:
                        for assign2 in assign_strings:
                            for assign3 in assign_strings:
                                ...
                                logging.debug("quadracity check "+ str(n_checked))
                                logging.debug('assign1: ' + assign1)
                                logging.debug('assign2: ' + assign2)
                                logging.debug('assign3: ' + assign3)
                                ...
                                ans_1q = self.iterate_cubically(maxterm, current_assignment)
                                #print(ans_1)
                                ...
                                ans_2q = self.iterate_cubically(maxterm, current_assignment)
                                #print(ans_2)
                                ...
                                ans_3q = self.iterate_cubically(maxterm, current_assignment)
                                #print(ans_3)


                                for index, secret_var in enumerate(self.bbpoly.secretvariables):
                                    current_assignment[secret_var] = int(assign3[index])

                                

                                ans_4q = self.iterate_cubically(maxterm, current_assignment)
                                #print(ans_4)


                                rhs_argument = format(format(int(assign1, 2) ^ int(assign2, 2) , 'b') ^ int(assign3, 2) , 'b')
                                ...
                                rhs_q = self.iterate_cubically(maxterm, current_assignment)
                                #print(rhs)

                                lhs_argument1 = format(int(assign1, 2) ^ int(assign2, 2), 'b')

                                lhs_argument2 = format(int(assign2, 2) ^ int(assign3, 2), 'b')

                                lhs_argument3 = format(int(assign1, 2) ^ int(assign3, 2), 'b')

                                while len(lhs_argument1) != self.degree:
                                    lhs_argument1 = '0' + lhs_argument1

                                while len(lhs_argument2) != self.degree:
                                    lhs_argument2 = '0' + lhs_argument2

                                while len(lhs_argument3) != self.degree:
                                    lhs_argument3 = '0' + lhs_argument3

                                current_assignment = {}

                                for index, secret_var in enumerate(self.bbpoly.secretvariables):
                                    current_assignment[secret_var] = int(lhs_argument1[index])

                                val1 = self.iterate_cubically(maxterm, current_assignment)

                                current_assignment = {}

                                for index, secret_var in enumerate(self.bbpoly.secretvariables):
                                    current_assignment[secret_var] = int(lhs_argument2[index])

                                val2 = self.iterate_cubically(maxterm, current_assignment)

                                current_assignment = {}

                                for index, secret_var in enumerate(self.bbpoly.secretvariables):
                                    current_assignment[secret_var] = int(lhs_argument3[index])

                                val3 = self.iterate_cubically(maxterm, current_assignment)

                                lhs_q = sum_mod2(sum_mod2(sum_mod2(sum_mod2(sum_mod2(sum_mod2(val3,val2),val1),ans_1q),ans_2q),ans_3q),ans_4q)

                                if lhs_q != rhs_q:
                                    quadracity_test = False
                                    q_flag = True
                                    break
    ...
    def find_superpoly(self, maxterm):
        ...
        current_assignment1 = {var:0 for var in secret_vars}

        current_assignment2 = {var:0 for var in secret_vars}
        ...
        for sv in secret_vars:
            for sv1 in secret_vars:
                ...
                current_assignment1[sv1] = 1

                value = self.iterate_cubically(maxterm, current_assignment)
                #print(value)

                value1 = self.iterate_cubically(maxterm, current_assignment1)
                #print(value)

                current_assignment2[sv]=1

                current_assignment2[sv1]=1 

                value2 = self.iterate_cubically(maxterm, current_assignment2)

                superpoly[sv+sv1] = constant_value ^ value ^ value1 ^ value2


                current_assignment[sv] = 0

                current_assignment1[sv1] = 0

                current_assignment2[sv] = 0

                current_assignment2[sv1] = 0
